...
describe("StreamingService", () => {
  ...
  beforeEach(async () => {
    ...
    mockEdgeCenterService = {
      getVideo: jest.fn() as jest.Mock<
        Promise<EdgeCenterVideoResponse>,
        [string]
      >,
      getThumbnailUrls: jest.fn() as jest.Mock<
        string[],
        [EdgeCenterVideoResponse]
      >,
    };
    mockStorageService = {
      fileExists: jest.fn().mockResolvedValue(false) as jest.Mock<
        Promise<boolean>,
        [string, string]
      >,
    };
  ...
  describe("getStreamUrl", () => {
    ...
    it("should not stream root-level video for series or tutorials", async () => {
      (mockPrisma.content!.findUnique as jest.Mock).mockResolvedValue({
        ...mockFreeContent,
        contentType: "SERIES",
    ...
    it("should not fall back from an episode to its root content video", async () => {
      (mockPrisma.content!.findUnique as jest.Mock).mockResolvedValue({
        ...mockFreeContent,
        contentType: "SERIES",
        edgecenterVideoId: null,
        videoFiles: [],
        series: { parentSeriesId: "root-series-id" },
      });

      await expect(service.getStreamUrl("content-123")).rejects.toThrow(
        NotFoundException,
      );
      expect(mockPrisma.content!.findUnique).toHaveBeenCalledTimes(1);
    });
  ...
  describe("verifyContentAccess", () => {
    ...
    it("should deny access without subscription", async () => {
      ...
      expect(result.reason).toBe(
        "Premium subscription or individual purchase required",
      );
    });
  });
