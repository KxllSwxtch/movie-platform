...
export class EdgeCenterWebhookController {
  ...
  async handleEncodingWebhook(
    ...
    try {
      ...
      if (!content) {
        this.logger.warn(
          `Content not found for EdgeCenter video: ${payload.video_id}`,
        );
        // Return 200 to prevent EdgeCenter from retrying
        return {
          received: true,
          message: "Content not found, webhook acknowledged",
        };
      }
  ...
  private async processWebhookEvent(
    ...
    const status = this.mapEdgeCenterStatusToEncodingStatus(
  ...
  private async handleEncodingComplete(
    ...
    const availableQualities = this.parseConvertedVideos(
      payload.converted_videos || [],
    );
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Update content with duration and generated thumbnail.
      // Keep manually uploaded covers intact.
      const thumbnailUrl =
        payload.poster || (payload.screenshots && payload.screenshots[0]);
      ...
      await tx.content.update({
        where: { id: contentId },
        data: {
          edgecenterClientId: "edgecenter",
          ...(payload.duration && { duration: Math.round(payload.duration) }),
          ...(thumbnailUrl &&
            !existingContent?.thumbnailUrl && { thumbnailUrl }),
        },
      });
    });
  ...
  private async handleEncodingFailed(
    ...
    const content = await this.prisma.content.findUnique({
      where: { id: contentId },
      select: { edgecenterClientId: true },
    });
    ...
    if (previousVideoId) {
      await this.prisma.content.update({
        where: { id: contentId },
        data: {
          edgecenterVideoId: previousVideoId,
          edgecenterClientId: "edgecenter",
        },
      });
      this.logger.error(
        `Encoding failed for content ${contentId}; restored previous EdgeCenter video ${previousVideoId}`,
      );
      return;
    }
  ...
  private async updateEncodingProgress(
    ...
    if (convertedVideos.length > 0) {
      const totalProgress = convertedVideos.reduce(
        (sum, v) => sum + v.progress,
        0,
      );
      const avgProgress = Math.round(totalProgress / convertedVideos.length);
      this.logger.log(
        `Encoding progress for content ${contentId}: ${avgProgress}%`,
      );
    }
