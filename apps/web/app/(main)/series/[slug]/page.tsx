...
export default function SeriesDetailPage() {
  ...
  const series = React.useMemo(() => {
    ...
    return {
      ...
      bannerUrl:
        d.bannerUrl || d.thumbnailUrl || "/images/movie-placeholder.jpg",
      seasonCount: d.seasonCount ?? d.seasons?.length ?? 0,
      episodeCount:
        d.episodeCount ??
        d.seasons?.reduce(
          (acc: number, s: { episodes?: unknown[] }) =>
            acc + (s.episodes?.length ?? 0),
          0,
        ) ??
        0,
      ...
      year:
        d.year ??
        (d.publishedAt ? new Date(d.publishedAt).getFullYear() : undefined),
      genres: d.genres ?? [],
      country: d.country,
      director: d.director,
      cast: d.cast ?? [],
  ...
  const nextEpisode = React.useMemo(() => {
    ...
    for (const season of series.seasons) {
      const next = season.episodes.find(
        (ep: EpisodeContent) => ep.isNext || (!ep.isWatched && !ep.progress),
      );
      if (next) return next;
    }
  ...
  if (error || !series) {
    return (
      <Container size="xl" className="py-16 text-center">
        <h1 className="text-2xl font-bold text-mp-text-primary mb-4">
          Сериал не найден
        </h1>
  ...
  return (
    <div className="bg-mp-bg-primary">
        ...
        >
          <div className="max-w-2xl">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {series.rating != null && (
                <RatingBadge rating={series.rating} size="lg" />
              )}
              {series.year && (
      ...
      <div className="relative z-20 bg-mp-bg-primary">
        ...
        {(series.director || series.country || series.cast.length > 0) && (
          <Container size="xl" className="pb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {series.director && (
                <div className="p-4 bg-mp-surface rounded-xl">
                  <p className="text-sm text-mp-text-secondary mb-1">
                    Режиссёр
                  </p>
            ...
            {series.cast.length > 0 && (
              <>
                <h3 className="text-lg font-medium text-mp-text-primary mt-6 mb-3">
                  В ролях
                </h3>
        ...
        <Container size="xl" className="space-y-8 py-8">
          <div className="space-y-2">
            <h2 className="text-xl font-semibold text-mp-text-primary">
              Отзывы и обсуждение
            </h2>
            <p className="text-sm text-mp-text-secondary">
              Оцените сериал и оставьте комментарий после просмотра.
            </p>
          </div>
          <ContentRating contentId={series.id} />
          <ContentComments contentId={series.id} />
        </Container>
      </div>
    </div>
  );
