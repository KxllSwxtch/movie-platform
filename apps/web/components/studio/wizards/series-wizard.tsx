...
import { seriesFormSchema, type SeriesFormValues } from '@/components/studio/schemas';
...
export function SeriesWizard({ onSuccess }: SeriesWizardProps) {
  ...
  React.useEffect(() => {
    const draftFields = new Set<keyof SeriesFormValues>([
    ...
    const subscription = watch((_values, { name }) => {
      if (!name || !draftFields.has(name as keyof SeriesFormValues)) return;
    ...
    return await new Promise<boolean>((resolve) => {
      createSeries.mutate(payload, {
        onSuccess: (data) => {
          ...
          toast.success('Черновик создан. Сезоны и серии можно добавлять в любое время.');
          resolve(true);
        },
        onError: () => resolve(false),
      });
    });
  }, [createdContentId, form, createSeries]);
    ...
    >
      ...
      {currentStep === 3 && createdContentId && (
        <div className="space-y-6">
          ...
          <StructuredContentVideoManager rootContentId={createdContentId} contentType="SERIES" />
        </div>
      )}
    </WizardShell>
  );
