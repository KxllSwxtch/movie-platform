...
export function StructuredContentVideoManager({
  ...
  const [episodeDrafts, setEpisodeDrafts] = React.useState<
    Record<number, EpisodeDraft>
  >({});
  const [editingEpisodeId, setEditingEpisodeId] = React.useState<string | null>(
    null,
  );
  const [editDraft, setEditDraft] = React.useState<EpisodeDraft>({
    title: "",
    description: "",
  });
  ...
  const handleDraftChange = React.useCallback(
    (seasonNumber: number, patch: Partial<EpisodeDraft>) => {
  ...
  const handleAddEpisode = React.useCallback(
    (season: SeriesSeason) => {
      const draft = episodeDrafts[season.seasonNumber] ?? {
        title: "",
        description: "",
      };
      ...
      <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#9ca2bc]">
            Добавляйте {isTutorial ? "главы и уроки" : "сезоны и серии"} в любой
            момент. Видео загружается только на уровне {itemLabel}.
          </p>
        </div>
        <Button
          type="button"
      ...
      <CardContent className="space-y-6">
        {seasons.length === 0 && (
          <div className="rounded-lg border border-dashed border-[#34394a] bg-[#080b12] p-5 text-sm text-[#9ca2bc]">
            Структура пока пустая. Нажмите «Добавить {groupLabel.toLowerCase()}
            », затем добавьте {itemLabel}.
          </div>
        )}
        ...
        {seasons.map((season) => {
          ...
          return (
            <section
              ...
              className="space-y-4 rounded-lg border border-[#272b38]/70 bg-[#080b12] p-4"
            >
              ...
              <div className="grid gap-3 rounded-md border border-[#272b38]/60 bg-[#10131c]/70 p-3 md:grid-cols-[1fr_1fr_auto]">
                <Input
                  value={draft.title}
                  onChange={(event) =>
                    handleDraftChange(season.seasonNumber, {
                ...
                {season.episodes.map((episode) => {
                  ...
                  return (
                    <div
                      ...
                      className="rounded-lg border border-[#272b38]/70 bg-[#0d1018] p-4"
                    >
                      {isEditing ? (
                        ...
                        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <p className="text-sm font-medium text-[#f5f7ff]">
                              {itemLabelCapitalized} {episode.episodeNumber}:{" "}
                              {episode.title}
                            </p>
                            {episode.description && (
                              <p className="mt-1 text-xs text-[#9ca2bc]">
                                {episode.description}
                              </p>
                            )}
                          </div>
