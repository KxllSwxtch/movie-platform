...
function normalizeAgeCategory(
  ...
  if (AGE_ENUM_MAP[value])
    return AGE_ENUM_MAP[value] as "0+" | "6+" | "12+" | "16+" | "18+";
  if (["0+", "6+", "12+", "16+", "18+"].includes(value))
    return value as "0+" | "6+" | "12+" | "16+" | "18+";
  return "0+";
}
...
export function DashboardRows({ data }: DashboardRowsProps) {
  const { continueWatching, trending, newReleases, series, tutorials, clips } =
    data;

  // Extract items from API response
  const continueItems: VideoProgressContent[] =
    continueWatching.data?.data?.items || [];
  const trendingItems: RatedMovieContent[] = (
    trending.data?.data?.items || []
  ).map(mapToRatedContent);
  const newItems: RatedMovieContent[] = (
    newReleases.data?.data?.items || []
  ).map(mapToRatedContent);
  const seriesItems: SeriesContent[] = (series.data?.data?.items || []).map(
    mapToSeriesContent,
  );
  const tutorialItems: TutorialContent[] = (
    tutorials.data?.data?.items || []
  ).map(mapToTutorialContent);
  const clipItems: ClipContent[] = (clips.data?.data?.items || []).map(
    mapToClipContent,
  );
