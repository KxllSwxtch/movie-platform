...
import { Archive, Eye, FilmStrip, PencilSimple } from '@phosphor-icons/react';
...
function getContentTypeBadge(type: string) {
  const config: Record<string, { label: string; className: string }> = {
    ...
    TUTORIAL: { label: 'Обучение', className: 'bg-purple-500/20 text-purple-400 border-transparent' },
  };
...
function getContentStatusBadge(status: string) {
  const config: Record<string, { label: string; className: string }> = {
    DRAFT: { label: 'Черновик', className: 'bg-gray-500/20 text-gray-300 border-transparent' },
    PUBLISHED: { label: 'Опубликовано', className: 'bg-green-500/20 text-green-400 border-transparent' },
    PENDING: { label: 'На модерации', className: 'bg-yellow-500/20 text-yellow-400 border-transparent' },
    REJECTED: { label: 'Отклонено', className: 'bg-red-500/20 text-red-400 border-transparent' },
...
export const contentColumns: ColumnDef<Content>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex max-w-[320px] flex-col">
        <span className="truncate font-medium text-mp-text-primary">{row.original.title}</span>
        <span className="truncate font-mono text-xs text-mp-text-disabled">{row.original.slug}</span>
      </div>
    ),
    enableSorting: true,
  },
  ...
  {
    id: 'topic',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Тематика" />,
    cell: ({ row }) => {
      const category = (row.original as Content & { category?: { name?: string } }).category;
      return <span className="text-sm text-mp-text-secondary">{category?.name ?? '—'}</span>;
    },
  },
  {
    accessorKey: 'status',
  ...
  {
    accessorKey: 'isFree',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Доступ" />,
    cell: ({ row }) =>
      row.original.isFree ? (
        <Badge className="border-transparent bg-emerald-500/20 text-emerald-400">Бесплатный</Badge>
      ) : (
        <Badge className="border-transparent bg-violet-500/20 text-violet-300">Платный</Badge>
      ),
  },
  {
    accessorKey: 'ageCategory',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Возраст" />,
    cell: ({ row }) => <AgeBadge age={row.original.ageCategory} />,
  },
  {
    accessorKey: 'viewCount',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Просмотры" />,
    cell: ({ row }) => <span className="text-mp-text-secondary">{formatNumber(row.original.viewCount)}</span>,
    enableSorting: true,
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm text-mp-text-secondary">
        {row.original.createdAt ? formatDate(row.original.createdAt as string | Date) : '—'}
      </span>
    ),
    enableSorting: true,
  },
...
function ContentRowActions({ row }: { row: { original: Content } }) {
  ...
  const isStructured = row.original.contentType === 'SERIES' || row.original.contentType === 'TUTORIAL';
    ...
    <DataTableRowActions
      ...
      actions={[
          ...
          : []),
        {
          ...
          icon: Archive,
          onClick: () => {
            window.dispatchEvent(new CustomEvent('admin:archive-content', {
