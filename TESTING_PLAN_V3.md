...
## Pre-Deploy Manual QA Checklist: Roles, Referrals, Withdrawals, Videos

**Status:** ready for manual QA
**Scope:** new CLIENT/PARTNER role boundaries, partner referrals, bonus withdrawals, `/videos` migration, build checks.

### CLIENT

- [ ] CLIENT does not see Studio UI or studio navigation.
- [ ] CLIENT does not see referral system UI or referral navigation.
- [ ] CLIENT cannot open protected partner/studio routes directly: `/studio`, `/partner`, `/account/referrals`, `/account/withdrawals`.
- [ ] CLIENT can browse content.
- [ ] CLIENT can buy paid content.

### PARTNER

- [ ] PARTNER sees the referral system.
- [ ] PARTNER sees their referral link.
- [ ] PARTNER can upload content.
- [ ] Uploaded PARTNER content goes to moderation.
- [ ] PARTNER cannot publish content directly without moderation.
- [ ] PARTNER sees bonus points balance.
- [ ] PARTNER can create a withdrawal request.

### REFERRAL / BONUS

- [ ] New CLIENT opens the app through PARTNER referral link.
- [ ] New CLIENT registers successfully.
- [ ] Referred CLIENT buys content.
- [ ] PARTNER receives bonus points for the purchase.
- [ ] Page refresh does not duplicate the bonus accrual.
- [ ] Repeated payment/update for the same purchase does not duplicate the bonus accrual.

### WITHDRAWALS

- [ ] PARTNER creates a withdrawal request.
- [ ] ADMIN sees the request at `/admin/bonuses/withdrawals`.
- [ ] `approve` action works and persists status.
- [ ] `reject` action works and returns reserved points to PARTNER balance.
- [ ] `complete` action works and closes the request correctly.

### VIDEOS

- [ ] `/videos` works as a standalone page.
- [ ] `/clips` redirects to `/videos`.
- [ ] Video categories render with icons.
- [ ] Search and category filters work together.
- [ ] Old "Клипы" mentions are not visible in the UI.
...
- Full API test suite is not green yet because of legacy unit tests.
- `noUnusedLocals` and `noUnusedParameters` are temporarily disabled in `apps/web/tsconfig.json`.
- Track both items as a separate cleanup task after this manual QA/deploy stage.

---

## Ручной QA-чек-лист: рефералы, баллы и вывод в рубли

**Метод:** только ручная проверка, без автотестов и тестового робота

**Цель:** пройти полный flow от реферальной ссылки партнёра до покупки клиентом, начисления баллов, конвертации в рубли, заявки на вывод и решения администратора.

**Рекомендуемые сессии:**
- Браузер A: сессия PARTNER
- Браузер B: сессия нового CLIENT, желательно инкогнито/приватное окно
- Браузер C: сессия ADMIN

### Тестовые аккаунты

| Роль | Аккаунт | Примечание |
|------|---------|------------|
| CLIENT | `user@movieplatform.local` / `user123` или новый клиент, зарегистрированный по реферальной ссылке | Обычный клиент |
| PARTNER | `partner@movieplatform.local` / `partner123` | Партнёр с доступом к реферальной системе |
| ADMIN | `admin@movieplatform.local` / `admin123` | Администратор для проверки заявок на вывод |

### Предусловия

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Открыть приложение в браузере A | Приложение загружается без критических ошибок | Не проверено |
| 2 | Подготовить отдельный браузер B или окно инкогнито с очищенными cookies/local storage | В браузере B нет активной авторизации | Не проверено |
| 3 | Подготовить браузер C или отдельный профиль для входа под ADMIN | Админская сессия не перетирает сессии партнёра и клиента | Не проверено |
| 4 | Выбрать платный контент, который может купить CLIENT | На странице контента есть действие покупки/оплаты | Не проверено |
| 5 | Зафиксировать начальный баланс баллов PARTNER до покупки | Начальный баланс известен для дальнейшего сравнения | Не проверено |
| 6 | Если система резервирует баллы при заявке на вывод, зафиксировать доступный и зарезервированный баланс отдельно | Доступный и зарезервированный балансы известны | Не проверено |

### 1. Проверка ролей

| # | Роль | Действие | Ожидаемый результат | Статус проверки |
|---|------|----------|---------------------|-----------------|
| 1 | CLIENT | Войти под CLIENT | CLIENT попадает в личный кабинет/дашборд без прав партнёра и администратора | Не проверено |
| 2 | CLIENT | Открыть основную навигацию/сайдбар/меню аккаунта | Пункт реферальной или партнёрской системы не отображается | Не проверено |
| 3 | CLIENT | Попробовать открыть партнёрские URL напрямую, например `/partner`, `/partner/referrals`, `/partner/withdrawals` | Доступ запрещён, пользователь перенаправлен или показано понятное состояние "нет доступа" | Не проверено |
| 4 | CLIENT | Попробовать открыть страницу отправки/создания контента, если URL известен | CLIENT не может отправлять контент; действие недоступно или доступ запрещён | Не проверено |
| 5 | CLIENT | Открыть страницу платного контента | CLIENT видит возможность купить контент | Не проверено |
| 6 | PARTNER | Войти под PARTNER | PARTNER попадает в личный кабинет/дашборд | Не проверено |
| 7 | PARTNER | Открыть навигацию/сайдбар/меню аккаунта | Пункт партнёрской/реферальной системы отображается | Не проверено |
| 8 | PARTNER | Открыть страницу реферальной системы | Страница открывается и показывает данные партнёра | Не проверено |
| 9 | ADMIN | Войти под ADMIN | ADMIN попадает в админку или видит админскую навигацию | Не проверено |
| 10 | ADMIN | Открыть админскую навигацию | Доступны разделы админки, включая заявки на вывод | Не проверено |

### 2. Проверка партнёра

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Под PARTNER открыть партнёрский дашборд/страницу рефералов | Доступ к реферальной системе есть | Не проверено |
| 2 | Проверить отображение реферальной ссылки | Ссылка видна и относится к текущему партнёру | Не проверено |
| 3 | Нажать кнопку копирования реферальной ссылки | Появляется успешный toast/сообщение или состояние копирования | Не проверено |
| 4 | Вставить скопированную ссылку в адресную строку или временное поле | Вставленная ссылка совпадает с отображаемой ссылкой | Не проверено |
| 5 | Проверить блок баланса баллов | Текущий баланс баллов отображается | Не проверено |
| 6 | Проверить блок истории начислений | История начислений отображается или корректно показан пустой список | Не проверено |
| 7 | Открыть вкладку/страницу вывода средств | Вкладка вывода средств доступна партнёру | Не проверено |
| 8 | Проверить форму или кнопку создания заявки на вывод | Партнёр может начать создание заявки на вывод | Не проверено |

### 3. Проверка клиента

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Под CLIENT открыть дашборд/навигацию аккаунта | Реферальная система не отображается | Не проверено |
| 2 | Под CLIENT попробовать открыть партнёрский URL напрямую | CLIENT не получает доступ к партнёрским функциям | Не проверено |
| 3 | Под CLIENT попробовать отправить или создать контент | CLIENT не может отправлять контент; кнопка/форма недоступна | Не проверено |
| 4 | Под CLIENT открыть каталог контента | Каталог успешно загружается | Не проверено |
| 5 | Под CLIENT открыть страницу платного контента | Цена и кнопка покупки отображаются | Не проверено |
| 6 | Начать сценарий покупки | CLIENT может перейти к оплате/checkout | Не проверено |

### 4. Проверка реферального перехода

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | В браузере A скопировать реферальную ссылку PARTNER | Ссылка успешно скопирована | Не проверено |
| 2 | Открыть скопированную ссылку в браузере B/инкогнито | Открывается реферальный landing/register flow без активной старой сессии | Не проверено |
| 3 | Зарегистрировать нового CLIENT с уникальным email | Регистрация проходит успешно | Не проверено |
| 4 | Пройти обязательные шаги профиля/верификации, если они есть | Новый CLIENT может пользоваться приложением | Не проверено |
| 5 | Под новым CLIENT открыть профиль/аккаунт | Роль аккаунта — CLIENT, не PARTNER и не ADMIN | Не проверено |
| 6 | Под PARTNER обновить список/историю рефералов | Новый клиент появился в рефералах или счётчик рефералов увеличился | Не проверено |
| 7 | Под ADMIN открыть карточку нового клиента, если доступно | В админке видно, что CLIENT привязан к PARTNER, либо есть запись об атрибуции | Не проверено |

### 5. Проверка покупки

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | В браузере B остаться авторизованным под новым реферальным CLIENT | Сессия CLIENT активна | Не проверено |
| 2 | Открыть выбранный платный контент | Страница контента загружается, покупка доступна | Не проверено |
| 3 | Запустить оплату/checkout | Открывается страница, модальное окно или flow оплаты | Не проверено |
| 4 | Завершить оплату доступным тестовым/песочными способом | Оплата становится успешной | Не проверено |
| 5 | Вернуться в приложение после оплаты | CLIENT видит успешное состояние покупки | Не проверено |
| 6 | Открыть покупки/библиотеку/заказы/историю оплат CLIENT | Покупка отображается в системе | Не проверено |
| 7 | Попробовать открыть или смотреть купленный контент | CLIENT получил доступ к купленному контенту | Не проверено |

### 6. Проверка начисления баллов

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | В браузере A обновить страницу баланса/рефералов PARTNER после успешной покупки CLIENT | Страница показывает актуальные данные партнёра | Не проверено |
| 2 | Сравнить текущий баланс баллов с начальным балансом | Баланс увеличился на ожидаемое количество баллов | Не проверено |
| 3 | Открыть историю начислений | В истории появилась новая запись по покупке CLIENT | Не проверено |
| 4 | Проверить детали записи начисления | Запись содержит покупку/клиента/дату/сумму или достаточно данных для идентификации покупки | Не проверено |
| 5 | Обновить страницу один раз | Баланс не увеличивается повторно от той же покупки | Не проверено |
| 6 | Обновить страницу ещё раз или открыть партнёрскую страницу в новой вкладке | Начисление отображается один раз; дублей в истории нет | Не проверено |
| 7 | Если доступен список платежей в админке, сравнить ID покупки/платежа с начислением | Начисление связано с той же успешной покупкой | Не проверено |

### 7. Проверка конвертации в рубли

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Открыть баланс/страницу вывода PARTNER | Количество доступных баллов видно | Не проверено |
| 2 | Проверить рублёвый эквивалент рядом с балансом или суммой вывода | Эквивалент в рублях отображается | Не проверено |
| 3 | Проверить текст курса, подсказку или формулу расчёта | Курс конвертации понятен, например `1 балл = 1 ₽` или другой явный курс | Не проверено |
| 4 | Ввести валидное количество баллов для вывода, если форма поддерживает ввод | Рублёвый эквивалент обновляется или остаётся легко проверяемым по курсу | Не проверено |
| 5 | Сравнить отображаемую сумму в рублях с курсом | Сумма в рублях рассчитана корректно | Не проверено |

### 8. Проверка заявки на вывод

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Под PARTNER открыть вкладку/страницу вывода средств | Видна форма или список заявок на вывод | Не проверено |
| 2 | Попробовать создать заявку на сумму больше доступного баланса | Форма блокирует отправку или API возвращает понятную ошибку валидации | Не проверено |
| 3 | Проверить текст ошибки | Ошибка объясняет, что сумма превышает доступный баланс | Не проверено |
| 4 | Ввести валидную сумму в пределах доступного баланса | Форма принимает сумму | Не проверено |
| 5 | Заполнить обязательные реквизиты, если они есть | Обязательные поля заполняются | Не проверено |
| 6 | Отправить заявку на вывод | Заявка успешно создаётся | Не проверено |
| 7 | Проверить появление заявки в списке выводов партнёра | Новая заявка отображается | Не проверено |
| 8 | Проверить статус заявки | Статус — `Ожидает` | Не проверено |
| 9 | Зафиксировать баланс после создания заявки | Понятно, списались баллы сразу или ушли в резерв | Не проверено |

### 9. Проверка админки

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Под ADMIN открыть страницу заявок на вывод, например `/admin/partners/withdrawals` | Страница модерации заявок загружается | Не проверено |
| 2 | Найти заявку PARTNER, созданную на предыдущем шаге | Заявка видна администратору | Не проверено |
| 3 | Открыть детали заявки или меню действий | Админские действия доступны | Не проверено |
| 4 | Одобрить заявку | Статус меняется на одобрена/выплачена/обработана согласно терминологии продукта | Не проверено |
| 5 | Обновить страницу админки | Новый статус сохраняется | Не проверено |
| 6 | Под PARTNER создать вторую валидную заявку для сценария отклонения | Вторая заявка появляется со статусом `Ожидает` | Не проверено |
| 7 | Под ADMIN найти вторую заявку | Вторая заявка видна администратору | Не проверено |
| 8 | Отклонить вторую заявку, указав причину, если поле обязательно | Статус меняется на отклонена | Не проверено |
| 9 | Обновить страницу админки | Статус отклонения сохраняется | Не проверено |
| 10 | Под PARTNER обновить список заявок на вывод | Оба решения администратора видны партнёру с корректными статусами | Не проверено |

### 10. Проверка итогового баланса

| # | Действие | Ожидаемый результат | Статус проверки |
|---|----------|---------------------|-----------------|
| 1 | Под PARTNER открыть баланс после одобрения заявки | Сумма одобренного вывода списана с доступного/общего баланса по правилам продукта | Не проверено |
| 2 | Проверить запись одобренного вывода в истории | Одобренная заявка остаётся в истории с финальным статусом | Не проверено |
| 3 | Под PARTNER открыть баланс после отклонения второй заявки | Если баллы резервировались при создании заявки, отклонённая сумма вернулась в доступный баланс | Не проверено |
| 4 | Проверить запись отклонённого вывода в истории | Отклонённая заявка остаётся в истории с финальным статусом и причиной, если она была указана | Не проверено |
| 5 | Сравнить итоговый баланс с ожидаемой формулой | Итоговый баланс = начальный баланс + начисление за покупку - одобренный вывод; отклонённый вывод баланс не уменьшает | Не проверено |
| 6 | Обновить страницы баланса и истории партнёра | Баланс и истории стабильны; нет дублей начислений или заявок | Не проверено |
| 7 | Под ADMIN обновить страницу заявок на вывод | Финальные статусы совпадают с тем, что видит PARTNER | Не проверено |

### Критерии прохождения

| Область | Критерий | Статус проверки |
|---------|----------|-----------------|
| Роли | CLIENT, PARTNER и ADMIN имеют корректные границы доступа | Не проверено |
| Реферал | Новый CLIENT, зарегистрированный по ссылке, привязан к PARTNER | Не проверено |
| Покупка | Реферальный CLIENT может купить контент, покупка записана в системе | Не проверено |
| Баллы | PARTNER получает ровно одно начисление за покупку | Не проверено |
| Конвертация | Баллы и рублёвый эквивалент видны, курс понятен | Не проверено |
| Вывод | PARTNER может создать валидную заявку и не может вывести больше доступного баланса | Не проверено |
| Админка | ADMIN может одобрить и отклонить заявки на вывод | Не проверено |
| Баланс | Одобренный вывод уменьшает баланс; отклонённый зарезервированный вывод возвращает баланс | Не проверено |
